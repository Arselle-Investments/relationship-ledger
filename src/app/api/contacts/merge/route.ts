...
export async function POST(req: NextRequest) {
  ...
  await prisma.$transaction(async (tx) => {
    // Everything that pointed at a secondary now points at the primary —
    // no unique constraints on any of these by contactId, so a plain
    // reassignment is safe (no collision handling needed, unlike company
    // merge's deal-outreach case).
    ...
    // Merge the secondaries' data into the primary wherever the primary is missing it.
    ...
    await tx.contact.update({
      ...
      data: {
        ...
        researchBio, researchBioSource, researchUpdatedAt,
        researchNews: researchNews === null ? Prisma.JsonNull : researchNews,
        agoraRaw: agoraRaw === null ? Prisma.JsonNull : agoraRaw,
      },
    });
