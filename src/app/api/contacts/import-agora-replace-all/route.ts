...
/**
 * Replaces the ENTIRE contact roster with a fresh Agora export — deletes
 * every contact first, so anything local-only (a Teams-confirmed contact not
 * yet pushed back to Agora, manual notes/tags/tasks on a record) is lost.
 * Admin-only and tucked into Settings rather than the Contacts toolbar on
 * purpose: this should be a rare, deliberate action (e.g. the very first
 * bulk load), not the everyday way to pull in new Agora entries — for that,
 * use the non-destructive POST /api/contacts/import-agora instead.
 */
