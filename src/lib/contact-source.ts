 ...
 * a display label from the same breadcrumbs the rest of the app already
 * relies on (agoraRaw presence, the import tags Agora imports/tag-sync
 ...
 */
export function contactSourceLabel(contact: Pick<Contact, "agoraRaw" | "tags">): string {
  const labels: string[] = [];
  ...
  if (labels.length === 0) return "Manually added";
